...
Chương này tổng hợp các phép đo định lượng đã có trong artifact huấn luyện và benchmark, cùng phép đo kiểm tra liên kết dữ liệu nghiệp vụ (audit bridge). Các con số dưới đây được trích từ nhật ký huấn luyện PhoBERT NER và các tệp JSON metric trong thư mục `reports/` (ví dụ `supa_metrics_run_*.json`, `supa_benchmark_aggregate_*.json`), trừ khi có ghi chú khác. **Mốc ghi nhận số liệu SUPA phân tầng trong tài liệu này:** **`reports/supa_benchmark_aggregate_stratified_k5_oracle_run56-60_20260513.json`** — aggregate **2026-05-12, 18:31 UTC**, các run trên cơ sở dữ liệu **2026-05-13, khoảng 01:04–01:07 (UTC+7)**. (Trong `reports/` vẫn có thêm file aggregate replicate oracle **n = 50** cũ hơn — không mô tả chi tiết trong chương.) Không có số liệu giả định: những hạng mục chưa có artifact đo được sẽ được nêu rõ là chưa báo cáo.
...
#### 9.10.1. Các tổ hợp thực nghiệm đề xuất

Chiến lược thực nghiệm SUPA-Bench cho phép đo lường hiệu quả của các kịch bản sau:

| Mã cấu hình | Thành phần kích hoạt | Mục tiêu đánh giá |
| :--- | :--- | :--- |
| **A1 (Full)** | NER + mGTE + LLM | Đánh giá hiệu năng tổng thể của kiến trúc Hybrid SOTA. |
| **A2 (No-LLM)** | NER + mGTE | Đánh giá độ chính xác khi chỉ dùng cơ chế retrieval và rule-based. |
| **A3 (Retrieval-only)** | mGTE (hoặc PhoBERT) | Đánh giá sức mạnh của embedding trong việc tìm kiếm địa chỉ chuẩn. |
| **A4 (NER + LLM)** | PhoBERT NER + Qwen3 | Đánh giá khả năng của LLM khi không có các ứng viên từ retrieval. |
| **A5 (Compare)** | NER + PhoBERT Siamese + LLM | So sánh hiệu quả giữa backbone PhoBERT và mGTE cho retrieval. |

#### 9.10.2. Phương pháp ghi nhận và báo cáo

Mỗi cấu hình thực nghiệm khi chạy qua `production_pipeline.py` sẽ tự động đánh dấu vào trường `processing_method` (ví dụ: `HYBRID_NER_MGTE_LLM`). Điều này cho phép bước `eval` của SUPA-Bench có thể:
1.  **Phân tích lỗi (Error Analysis):** Xác định loại nhiễu nào (D1-D4) làm khó mô hình nào nhất.
2.  **Đánh giá đánh đổi (Trade-off):** So sánh giữa độ chính xác (Exact Match) và tài nguyên tính toán (độ trễ khi có/không có LLM).
3.  **Chứng minh tính vượt trội:** Khẳng định việc kết hợp NER (để bóc tách context) và Retrieval (để lấy ứng viên) giúp LLM đạt kết quả ổn định hơn so với việc chạy LLM đơn thuần trên địa chỉ nhiễu.

Kết quả của các nghiên cứu này là cơ sở quan trọng để khẳng định **khoảng trống nghiên cứu** về tính nhất quán địa giới và khả năng chịu lỗi của hệ thống trong môi trường dữ liệu Việt Nam phức tạp.
...
**Chưa hoàn tất trong báo cáo số:** bảng E2E theo kịch bản S1–S3 trên địa chỉ thực tế; cột retrieval trong tổng hợp SUPA — cần chạy `evaluate_retriever` với `--persist-db` trên snapshot được chọn. Chuỗi **phân tầng K = 5, N = 2000** (oracle) đã chạy và tổng hợp trong `reports/supa_benchmark_aggregate_last.json` cùng persist **`ath.supa_stratified_eval_summary`** (mục 9.5.1); **còn thiếu** toàn bộ chỉ số **trên pipeline chuẩn hóa thật** (không oracle) để đối chiếu bảng ngưỡng kỳ vọng, và số **latency/throughput của pipeline** (cần cột **`latency_ms`** trong CSV preds từ normalizer) — số đo **ingest DB** từ `import-preds` khi CSV thiếu cột chỉ bổ trợ kỹ thuật, không thay thế báo cáo hiệu năng inference.
...
**Chi phí và độ trễ.** Pipeline đầy đủ (nhúng corpus, NER, LLM) có chi phối GPU/CPU và thời gian khởi động; trade-off đã được nhận thức (corpus limit, quantization) nhưng cần bảng đo trên phần cứng mục tiêu triển khai.
