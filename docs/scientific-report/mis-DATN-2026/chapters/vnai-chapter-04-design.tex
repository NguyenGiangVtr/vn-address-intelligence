...
\textbf{Profile nhiễu SUP-1.0.0.} Hàm sinh nhiễu deterministic theo \macode{random.Random(seed)} áp bốn loại perturbation phổ biến: (i) thêm tiền tố ngẫu nhiên kiểu ``Gần '', ``Đối diện '', ``Bên cạnh '' (mô phỏng cách người dùng mô tả vị trí tương đối); (ii) thêm hậu tố ghi chú kiểu ``(gặp bảo vệ)'', ``- gọi trước'' (mô phỏng instruction giao hàng); (iii) thay ``, '' bằng biến thể có khoảng trắng thừa hoặc thiếu (\macode{",  "}, \macode{","}); (iv) với xác suất nhỏ, gấp đôi khoảng trắng nội bộ. Tính chất deterministic của seed đảm bảo mọi lần chạy với cùng seed sinh cùng tập specimen, là điều kiện cần cho khả năng tái lập.
