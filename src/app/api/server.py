...
from app.api.supa_benchmark_ui import router as supa_benchmark_ui_router
...
from app.paths import ai_config_yaml_relative_posix, repo_root

_DEFAULT_AI_YAML_REL = ai_config_yaml_relative_posix()
...
from app.api.job_runners import run_benchmark_job, run_batch_job, run_osm_job  # noqa: E402
from app.api.state import (  # noqa: E402
    batch_job_lock,
    batch_job_state,
    benchmark_job_lock,
    benchmark_job_state,
...
def trigger_benchmark_job(data: dict = None, current_user: str = Depends(get_current_user)):
    ...
    payload = data or {}
    config_path = payload.get("config_path", _DEFAULT_AI_YAML_REL)
...
def _get_evidence_dirs():
    r = repo_root()
    return [r / "reports" / "evidence_real", r / "reports" / "evidence"]
...
class PreLabelerExportPayload(BaseModel):
    limit: int = 500
    config_path: Optional[str] = Field(default_factory=ai_config_yaml_relative_posix)
...
def export_prelabeler_label_studio(payload: PreLabelerExportPayload, current_user=Depends(get_current_user)):
    ...
    config_path = str(payload.config_path or _DEFAULT_AI_YAML_REL).strip()
...
api_router.include_router(supa_benchmark_ui_router, prefix="/experiments")
